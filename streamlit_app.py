...
.bar-label-row {
    ...
    font-size: 12px; color: #6e6e73; margin-bottom: 4px;
}
...
.section-title {
    ...
    color: #1d1d1f; margin: 8px 0 16px 0;
}
...
<div style="margin-bottom:28px;">
  ...
  <div style="font-size:14px;color:#6e6e73;margin-top:2px;">Polymarket 배팅 판단 도구 · 수동 거래 기록장</div>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    st.markdown('<div class="muted" style="margin-bottom:16px;">진입 전 5가지 숫자만 넣으면 바로 판단해 드립니다.</div>', unsafe_allow_html=True)
    ...
    with st.form("quick_form"):
        ...
        with c2:
            ...
            q_bankroll = st.number_input("포트폴리오 총액 ($)", 1.0, value=1000.0)
    ...
    if live_submitted:
        if not l_token.strip():
            ...
            try:
                ...
                if sell_p is None:
                    st.error("가격 정보를 가져오지 못했습니다. token_id를 확인해 주세요.")
                else:
                    ...
                    elif sell_p >= l_target/100: decision, level = "목표가 도달 — 매도 또는 부분매도 고려", "warn"
