...
.result-card { border-radius: 16px; padding: 22px; margin: 16px 0; }
...
.ai-section { background: #f5f5f7; border-radius: 12px; padding: 14px; margin-bottom: 10px; }
.ai-section-label { font-size: 11px; font-weight: 600; color: #6e6e73; text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 6px; }
.ai-section-content { font-size: 14px; color: #1d1d1f; line-height: 1.6; white-space: pre-wrap; }
...
def partial_rows(shares, price_dec, investment):
    rows, min_r = [], None
    if price_dec and shares and price_dec > 0:
...
def call_claude(team_a, team_b, league, current_price, fair_price, purpose):
    # API 키 가져오기
    api_key = None
    ...
    if not api_key:
        return "❌ API 키를 찾을 수 없습니다.\nStreamlit Secrets에 ANTHROPIC_API_KEY를 저장해주세요."
...
경기: {team_a} vs {team_b}
리그: {league}
{team_a} 현재 배당: {current_price}¢
내가 생각하는 적정가: {fair_price}¢
배팅 목적: {purpose}

아래 6가지 항목을 분석해줘. 각 항목 앞에 번호 붙여줘.

1. 리그 순위 ({team_a} vs {team_b} 현재 순위)
2. 이번 시즌 전적 (각 팀 승/패 기록)
3. 최근 5경기 폼 (연승/연패, 최근 흐름)
4. 상대 전적 (두 팀 직접 맞대결 역사)
5. 팀 상태 (로스터, 부진 선수, 특이사항)
6. 배팅 관점 ({current_price}¢가 적절한지, 엣지가 있는지)

마지막 줄에 이것만 써줘:
결론: 배팅 추천 / 비추천 / 중립

한국어로 답해줘. 각 항목은 2~3줄로 간결하게."""
...
with tab1:
    ...
    with left_col:
        ...
        with st.form("quick_form"):
            ...
            c1, c2 = st.columns(2)
            with c1:
                q_team_a = st.text_input("내가 배팅하는 팀", "T1")
            with c2:
                q_team_b = st.text_input("상대 팀", "HLE")
            ...
            submitted = st.form_submit_button("판독하기 + AI 분석 요청", use_container_width=True)
        ...
        if submitted:
            ...
            # AI 분석 호출
            with st.spinner("🤖 AI 팀 분석 중... (5~10초 소요)"):
                try:
                    ...
                    st.session_state.ai_result = f"분석 실패: {str(e)}"
    ...
    with right_col:
        ...
        if st.session_state.ai_result:
            if st.session_state.ai_result.startswith("❌") or st.session_state.ai_result.startswith("분석 실패"):
                st.error(st.session_state.ai_result)
            else:
                lines = st.session_state.ai_result.strip().split("\n")
                current_title = ""
                current_body = []
                    ...
                    if len(line) > 2 and line[0].isdigit() and line[1] == ".":
                        if current_title:
                            ...
                            <div class="ai-section">
                                ...
                                <div class="ai-section-content">{chr(10).join(current_body)}</div>
                            </div>
                            """, unsafe_allow_html=True)
                        current_title = line
                        current_body = []
                        ...
                        if current_title:
                            ...
                            """, unsafe_allow_html=True)
                        # 결론은 크게 표시
                        if "추천" in line and "비추천" not in line:
                            st.markdown(f'<div class="result-card result-good"><div class="result-title">✅ {line}</div></div>', unsafe_allow_html=True)
                        elif "비추천" in line:
                            st.markdown(f'<div class="result-card result-bad"><div class="result-title">⛔ {line}</div></div>', unsafe_allow_html=True)
                        else:
                            st.markdown(f'<div class="result-card result-warn"><div class="result-title">⚠️ {line}</div></div>', unsafe_allow_html=True)
                        current_title = ""
                        current_body = []
                    else:
            ...
            <div style="text-align:center;padding:60px 20px;color:#6e6e73;">
                ...
                <div style="font-size:15px;font-weight:500;color:#1d1d1f;margin-bottom:8px;">AI 분석 대기 중</div>
                <div style="font-size:13px;">왼쪽에서 팀 입력 후<br>판독하기 버튼을 누르면<br>자동으로 분석됩니다.</div>
            </div>
            """, unsafe_allow_html=True)
...
with tab2:
    ...
    with st.form("live_form"):
        ...
        with c1:
            ...
            l_outcome  = st.text_input("선택한 팀", "KT Rolster")
        ...
        live_ok = st.form_submit_button("가격 조회 + 판정", use_container_width=True)
...
with tab3:
    ...
    if st.button("손익 계산하기", use_container_width=True):
        ...
        klass, icon, _ = result_cls("good" if profit >= 0 else "bad")
        st.markdown(f"""
        <div class="result-card {klass}">
...
with tab4:
    ...
    with c2: s_cur    = st.number_input("현재가 (¢)", 1.0, 100.0, 58.0, key="s_cur")
...
with tab5:
    ...
    with c3: r_deposit = st.number_input("추가 입금/출금 ($)", value=0.0)
    ...
    if st.session_state.trade_log:
        ...
        st.markdown('<div class="muted">아직 기록이 없습니다. 손익 계산 탭에서 계산하면 여기에 쌓입니다.</div>', unsafe_allow_html=True)